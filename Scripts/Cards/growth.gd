...
func execute_action(container):
	...
	var acc_check = await randi_range(1,100) <= container.combatants[container.currTurn].check_accuracy(100, "earth")
	if acc_check:
		...
		container.combatants[randi_range(0,len(container.combatants) - 1)].add_effect("modifier attack", .5, "earth", 0)
