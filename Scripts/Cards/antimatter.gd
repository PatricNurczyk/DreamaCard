...
func execute_action(container,acc):
	...
	if acc_check:
		container.combatants[container.target].add_effect("modifier attack", .35, "void", -1)
	else:
		...
		container.combatants[randi_range(0,len(container.combatants) - 1)].add_effect("modifier attack", .35, "void", -1)
